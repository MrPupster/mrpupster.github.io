...
//                 var divArt = $('<div>').addClass('article').append(agrupador);
	     	var yyy = $('<div>').addClass('yyy' + ano).append(agrupador);
                $(eTimeline).append(yyy.append($('<div>').addClass('article')));
...
//         $('#list0').data('y', {2019: 4});
		...
		$.each(eTimeline.find('div[class*="group"]'), function (index, value) {
		 	if (settings.effect && settings.effect != 'none') $(this).addClass('animated fadeInLeft').css('animation-duration', '0.5s')			
            	})
            if (settings.showMenu) {
		    eMenuD.append(eMenu);
		    if (settings.effect && settings.effect != 'none') eMenuD.slideUp()
