...
//             var irmaos = agrupador.siblings('article[id^="a' + ano + '"]');
            ...
            if (irmaos.length > 0) slot.insertAfter(irmaos.last());
                      ...
                      if (agrupador.length === 0) {
                agrupador = $('<div data-toggle="tooltip" data-placement="right" title="текст всплывающей подсказки">').attr('id', ('y' + ano)).addClass('group' + ano).text(ano);
                $(eTimeline).append(agrupador);
//                 var anchor = $('<a>').attr('href', ('#y' + ano)).text(ano).attr('title', $('#list0').data('y'));
                var anchor = $('<a>').attr('href', ('#y' + ano)).text(ano).attr('title', 'a');
                eMenu.append($('<li>').append(anchor))
            }
...
//         $('#list0').data('y', {2019: 4});
    ...
    document.getElementById('#y2018').setAttribute('title', 'aaa');
    $('#y2019').attr('title', 'word');
})(jQuery);
