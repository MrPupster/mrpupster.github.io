...
//             var irmaos = agrupador.siblings('article[id^="a' + ano + '"]');
            ...
            if (irmaos.length > 0) slot.insertAfter(irmaos.last());
                      ...
                      if (agrupador.length === 0) {
                agrupador = $('<div>').attr('id', ('y' + ano)).addClass('group' + ano).text(ano).attr('name', ('y' + ano)).attr('data-toggle', 'tooltip');
                $(eTimeline).append(agrupador);
//                 var anchor = $('<a>').attr('href', ('#y' + ano)).text(ano).attr('title', $('#list0').data('y'));
                var anchor = $('<a>').attr('href', ('#y' + ano)).text(ano).attr('name', ('y' + ano)).attr('data-toggle', 'tooltip1');
                eMenu.append($('<li>').append(anchor))
            }
...
//         $('#list0').data('y', {2019: 4});
        ...
        $.each(qqq, function (key, value) {
                $('[name="y' + key + '"]').attr('title', 'Всего стримов в ' + key + '-м году: ' + value);
            })
